...
The lognormal survival model assumes that the time-to-event variable is lognormally distributed. The duration variable of the breastfeeding data set is fairly well represented by a normal distribution following log-transformation (Supplementary Figure S1). As an AFT model, differences between groups should be able to be modelled as accerlated time. Figure 5 shows the lognormal model provides a fit to the data qualitatively similar to the other parametric models above. Trends between the different racial groups correspond to regression outputs below showing similar survival for these three groups.
...
bfeed_og %>%
    ...
    finalfit::finalfit(dependent, explanatory) -> t

t %>% knitr::kable()
```

#TODO: table above looks really good! change around the second column of t to make names pretty, read through tutorial website on how to make it a bit prettier, then include it here in the paper! Make sure to set the reduced model with explanatory_multi = c() to get the optimized model from above to be the multi() output.
...
coxph.full.model <- coxph(SurvObj ~ race + poverty + smoke + ybirth + yschool, data = bfeed) 
...
ggcoxdiagnostics(coxph.full.model, type = "schoenfeld")

#alternative code to above line  (I think)
ggcoxzph(cox.zph(coxph.full.model))
```
...
```{r, echo=FALSE}
# comparison of coefficient estimates across the parametric models
