...
summary_expAIC %>%
  knitr::kable(digits=5,caption="Exponential survival model with lowest AIC")
```


#### Parameter Estimates\

The parameter estimates for the best fit exponential model are shown in Table 5. We see p-values below the 0.05 threshold for all parameters except for where race is black has a p-value higher than 0.05.
...
S(t) = e^{-\lambda t}
$$
where $h(t)$ is the hazard function and $S(t)$ is the survival function. These are the AFT parameterizations of the exponential model. Note that the the hazard is monotonic for this model. When $\alpha=1$, the Weibull model is the exponential model.
...
```{r, echo=FALSE}
Weibull.aft.ph <- -coef(best.Weibull.model)/best.Weibull.model$scale
#https://myweb.uiowa.edu/pbreheny/7210/f15/notes/10-15.pdf
Weibull.aft.ph %>%
